---
图表类型: 标签页 (Tab)
功能标签: [标签页组合, 多图表切换, Bar图表, Line图表, Pie图表, Grid图表]
数据量级标签: small, medium, large
适用场景: 需要在同一页面展示多个不同类型图表，通过标签页切换查看的场景。
数据适应: 多种类型图表的组合展示，数据格式根据各图表类型而定。
美观要点: 标签页清晰、图表布局合理、切换流畅、各图表样式统一。
---

### 基础标签页组合

这段代码展示了如何创建一个标签页组件，将多个不同类型的图表组合在一起，通过标签页切换查看。
