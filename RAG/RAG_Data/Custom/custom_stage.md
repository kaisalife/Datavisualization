---
图表类型: 自定义阶段图 (Custom Stage)
功能标签: [时间轴, 阶段展示, 自定义图表, 睡眠阶段, 时间范围]
数据量级标签: small
适用场景: 展示时间轴上的不同阶段，如睡眠阶段、任务状态、项目进度等。
数据适应: 阶段数量 3-10 之间效果最佳，每个阶段有开始和结束时间。
美观要点: 阶段颜色区分明确，时间轴刻度清晰，阶段范围展示直观。
---

# 自定义阶段图 (Custom Stage)

展示时间轴上不同阶段的自定义图表，如睡眠阶段分析。
